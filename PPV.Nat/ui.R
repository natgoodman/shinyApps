	...
	fluidRow(
		...
		column(4,
			...
			sliderInput("alpha", HTML("&alpha; level"), min = 0.01, max = 0.1, value = 0.05, step = 0.01),
                       ...
                       p("Do you want to specify power directly or indirectly by specifying sample size and effect size?", style = "font-style: italic; font-size: 0.85em; color:grey"),
                       radioButtons("power_select", label=NULL,
                                    choiceValues=list("power", "nd"),
                                    choiceNames=list(HTML('<span style = "font-size: 0.85em">specify power (&beta;) directly</span>'), list(HTML('<span style = "font-size: 0.85em">specify power indirectly through sample size (n) and effect size (d)</span>')))),
                       ## p(HTML("What is the power (&beta;) of the study?"), style = "font-style: italic; font-size: 0.85em; color:grey"),
                       sliderInput("power", label = HTML("Power (&beta;)"), min = 0.01, max = 0.99, value = 0.35, step = 0.01),
                       ## p("What is your sample size (n) per group?", style = "font-style: italic; font-size: 0.85em; color:grey"),
                       sliderInput("n", label = "Sample size (n)", min = 1, max = 100, value = 16, step = 1),
                       ## p("What is the standardized effect size (d)?", style = "font-style: italic; font-size: 0.85em; color:grey"),
                       sliderInput("d", label = "Effect size (d)", min = 0.1, max = 2, value = 1, step = .1),
